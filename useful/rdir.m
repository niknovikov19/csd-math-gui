function [varargout] = rdir(lev, rootdir, varargin)
...
if lev > 10
	fprintf('Max. level reached:\n%s\n', rootdir);
	varargout{1} = struct('name',{},'date',{},'bytes',{},'isdir',{},'datenum',{});
    return;
end	
...
elseif strcmp(wildpath,'**'), % a double wild directory means recurs down into sub directories
    ...
    D = rdir(lev+1, [prepath postpath(2:end)]);
    ...
    for ii = 1:length(tmp),
        if (tmp(ii).isdir && ~strcmpi(tmp(ii).name,'.') && ~strcmpi(tmp(ii).name,'..') ),
            Dt = [Dt; rdir(lev+1, [prepath tmp(ii).name filesep wildpath postpath])];
        end;
    end;
    D = [D; Dt];
else
    ...
    for ii = 1:length(tmp),
        if (tmp(ii).isdir && ~strcmpi(tmp(ii).name,'.') && ~strcmpi(tmp(ii).name,'..') ),
            D = [D; rdir(lev+1, [prepath tmp(ii).name postpath])];
        end;
    end;
end;
