...
for k = 1 : Nc
	...
	r1=c(k)*r;
	y1=y1+r1;
	if (abs(r1/y1) <= eps && k > klim)
		%fprintf('c eps reached\n');
		break;
	end
end
...
for k=1:Nd
	...
	r1=d(k+1)*r;
	y2=y2+r1;
	if (abs(r1/y2)<=eps && k > klim)
		%fprintf('d eps reached\n');
		break;
	end
end
...
if a < 0 && (a+1/2)==fix(a+1/2)
	ar=pi*(1/4+a/2);
	f1=cgamma2(1/4-a/2)/(sqpi*2^(a/2+1/4));
	f2=cgamma2(3/4-a/2)/(sqpi*2^(a/2-1/4));
